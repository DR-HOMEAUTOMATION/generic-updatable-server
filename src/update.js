class Updater {
     ...
     * @property  {!string} start_script_path - path to the startup script for the current program  
    */
    ...
    constructor(activeServers,gitConfig){
        ...
        if(!gitConfig.repository || !gitConfig.backup_path || !gitConfig.start_script_path) throw new Error('The gitConfig must contain (at least) : repo,backup_path,start_script_path')
        this.activeServers = activeServers;
        this.initGitUpdater(gitConfig); 
    }
    ...
    async initGitUpdater(config){
         this.AutoGitUpdater = new (await import('auto-git-update')).default({...config,tempLocation:config.backup_path})
    }
    ...
    */
    forceUpdate(){
        // const shutdownServers = this.activeServers.map((s={server,sockets})=>{
